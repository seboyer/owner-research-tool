-- Migration 020 — zapier_enriched_contacts: LLC-chain proxy + employs backfill
--
-- Builds on migration 019.  Two changes:
--
-- 1. Backfill 'employs' edges from existing data.  For every individual
--    who has an 'owned_by' relationship to an LLC that has an 'operates_as'
--    parent (the operating company), write the implied employs edge:
--      parent = operating company   child = individual signer/owner
--
--    Migration 002 defined 'employs' as an allowed relationship_type but
--    no code path ever wrote one — discovered while debugging the
--    zapier_enriched_contacts view's contact_company column.  Forward
--    writes are now added in enrichment/contact/orchestrator.py; this
--    step covers the existing data.
--
-- 2. Add an LLC-chain LATERAL as another COALESCE branch for
--    contact_company.  Defensive cover: if a future code path ever writes
--    'operates_as' without writing the matching 'employs' (they live in
--    different files), the view still resolves the company correctly.

BEGIN;

-- ── Step 1: Backfill 'employs' edges ────────────────────────────────────────
INSERT INTO entity_relationships (
    parent_entity_id, child_entity_id, relationship_type,
    source, confidence, evidence
)
SELECT DISTINCT
    er_op.parent_entity_id          AS parent_entity_id,   -- operating company
    er_own.parent_entity_id         AS child_entity_id,    -- individual signer/owner
    'employs'                       AS relationship_type,
    'backfill_owned_by_operates_as' AS source,
    0.70                            AS confidence,
    'Backfilled: individual is owner/signer of an LLC operated by this company' AS evidence
FROM entity_relationships er_own
JOIN entity_relationships er_op
  ON er_op.child_entity_id   = er_own.child_entity_id      -- same LLC
 AND er_op.relationship_type = 'operates_as'
JOIN entities owner_e
  ON owner_e.id = er_own.parent_entity_id
WHERE er_own.relationship_type = 'owned_by'
  AND owner_e.entity_type      = 'individual'              -- only persons get employed
  AND er_own.parent_entity_id <> er_op.parent_entity_id    -- avoid self-employs
ON CONFLICT (child_entity_id, parent_entity_id, relationship_type) DO NOTHING;

-- ── Step 2: Replace view with LLC-chain proxy fallback ──────────────────────
...
SELECT
    ...
    c.confidence,
    -- Resolve contact_company in priority order:
    --   1. Direct 'employs' edge (canonical, written by orchestrator + backfill)
    --   2. LLC chain (signer owns LLC, LLC operates_as company) — proxy fallback
    --   3. The entity itself (when ultimate parent is genuinely a person)
    COALESCE(employer.name,        llc_chain.name,        e.name)        AS contact_company,
    COALESCE(employer.entity_type, llc_chain.entity_type, e.entity_type) AS contact_company_type,
    COALESCE(employer.website,     llc_chain.website,     e.website)     AS contact_company_website,
...
LEFT JOIN entities e
    ON e.id = c.entity_id
-- Tier A: direct 'employs' edge (parent = company, child = person)
LEFT JOIN LATERAL (
...
-- Tier B: LLC-chain proxy — person owns LLC, LLC operates_as company
LEFT JOIN LATERAL (
